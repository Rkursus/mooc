...
PASS="OK"
PRECHECK_URL_OPTION="--precheck-url"
...
def main(argv):
    ...
    if PRECHECK_URL_OPTION in params:
        i = params.index(PRECHECK_URL_OPTION)
        precheck_url = params[i+1]
        del params[i+1] 
        del params[i]
        
        precheck_result = _precheck(precheck_url)
        if "ERROR" in precheck_result:
            _print_msg(precheck_result["ERROR"])
            return
        else:
            ...
            if "COMMENT" in precheck_result and precheck_result["COMMENT"].strip():
                _print_msg(precheck_result["COMMENT"], "Info")
...
def _format_message(message, exc_type, traceback,
    ...
    if (exc_type == "AssertionError" and message):
...
def _precheck(url):
    files = {}
    for filename in get_submitted_files():
        try:
    ...
    if "MOODLE_USER_NAME" in data:
        data["CLEAN_MOODLE_USER_NAME"] = _clean_name(data["MOODLE_USER_NAME"])
