...
Sageli kõige tüütum andmeanalüüsi juures on tulemuste vormistamine ilusaks dokumendiks. R-i väljund on tavaliselt kas konsoolis või graafikaaknas. Isegi kui tulemused on konsooliaknas tabelina, on seda Wordi vms programmi ümber tõsta tülikas. Õnneks on ka selle probleemi lahendamiseks R-is mitmeid erinevaid pakette, üks neid on `knitr`, mis defineerib dokumendikeele `Rmarkdown` ja RStudio on selle paketiga töötamise teinud äärmiselt mugavaks.

## Tulemuste vormistamine RStudio ja `Rmarkdown` abil

`Rmarkdown` on R-i võimalustega laiendatud [_markdown_](https://et.wikipedia.org/wiki/Markdown) keel. Kui oled varem _markdown_ keelega kokku puutunud, siis enamus reeglid ja süntaksit on sulle tuttavad.

`Rmarkdown` keeles tulemuste vormistamise tööpõhimõte on järgmine:
 ...
 * [RStudio `R markdown` õppematerjale](https://rmarkdown.rstudio.com/lesson-1.html)
 * [Olulisemad vormindusvõtted](http://rmarkdown.rstudio.com/authoring_basics.html)
 * [R markdown _cheat sheet_](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf)
 * [Knitr paketi autori koduleht](http://yihui.name/knitr)
 * [Põhjalik R Markdown õpik (tasuta)](https://bookdown.org/yihui/rmarkdown/)
 

### Ülesanded

1. Tee oma markdown dokument. Selleks mine `File -> New file -> R Markdown` ja sulle avaneb aken algseadistustega. Lisa dokumendi pealkiri (nt. `Minu R markdown testimine`), enda nimi autori lahtrisse ja vali HTML dokument väljundiks (ole mureta, kõike saab hiljem muuta). Sulle avaneb juba valmis sisuga dokument, mis annab ülevaate peamistest R markdown võimalustest javormistamisviisist.
1. Salvesta oma uus dokument meelepärasesse kataloogi.
1. Vajuta `Knit` nuppu lõngakera ikooniga (see asub skriptiakna ülemises servas). Võrdle väljasülitatud dokumenti sinu salvestatud R markdown dokumendiga.

    * Kuidas on sinu dokumendis märgitud pealkiri? Mis sümbol eelneb pealkirja tekstile?
    * Mis sümbolid ümbritsevad seda koodiosa, mis annab välja kokkuvõtva andmestiku tabeli?
    * Mis juhtub tekstiga kui see panna graavise (_backtick_) ehk " \` " sümbolite vahele?
    * Kuidas saab markdown dokumendis jooniseid lisada?
    * Mis juhtub tekstiga kui see panna " \*\* " sümbolite vahele?
  
1. Loe R koodi jupis (_chunk_) sisse `mass` andmestik (vt. eelmisi praktikume). Lisa koodijupi alguses loogeliste sulgude vahele sellised argumendid, et koodi ei trükita dokumenti (vaikimisi trükitakse).
1. Lisa allolev seletav tekst, kus `<x>` ja `<y>` asemel oled arvutanud vastavad arvud otse koodi sees kasutades selleks funktsioone `nrow(.)` ja `ncol(.)`:\
`"Andmestikus mass on <x> rida ja <y> veergu."`\
Otse teksti sees arvutuste tegemiseks kasuta järgnevat kirjutusviisi graavise sümbolite vahel `r knitr::inline_expr("ncol(Brand19)")`.
1. Lisa dokumenti tunnuse `AGEP` histogramm.
1. Lisa dokumenti tunuste `SEX`, `MAR` ja `WAGP` kokkuvõttev statistika (nt. käsuga `summary(.)`). Vorminda tabel kasutades funktsiooni `kable(.)` või `xtable(.)`.
