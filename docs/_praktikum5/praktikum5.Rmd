...
andmed <- read.table("https://github.com/Rkursus/mooc/raw/main/data/mass.txt", sep="\t", header=T, stringsAsFactors = T)
mk <- read.table("https://github.com/Rkursus/mooc/raw/main/data/maakonnad.txt", sep = " ")
library(ggplot2)
```
...
### Pakett `ggplot2` ja graafikute grammatika
...
Paketi `ggplot2`  käskude jaoks on kõige põhjalikum dokumentatsioon internetis aadressil: [http://ggplot2.tidyverse.org/reference/](http://ggplot2.tidyverse.org/reference/). Ühtlasi leidub mitmeid inglisekeelseid raamatuid, mis õpetavad `ggplot2` peensusi. Kaks tasuta e-raamatut on näiteks W. Chang'i [R Graphics Cookbook](https://r-graphics.org/) ja K. Healy [Data visualization](https://socviz.co/).
...
visiidid <- read.table("https://github.com/Rkursus/mooc/raw/main/data/visiidid.txt", sep = "\t", header = TRUE)[1:5, ]
str(visiidid)
...
<!--1. Loe sisse andmestik: `read.table("https://github.com/Rkursus/mooc/raw/main/data/maakonnad.txt", sep = " ", header=T)`-->
