...
- **Ülesanne 3** Võrdle kahte tulemust, milles seisneb erinevus? Vali õige väide:
  
    1. Esimeses andmestikus on sorteeritud andmestik `y`-i järgi kasvavalt, teises andmestikus on `y`-i järgi kahanevalt ning seejärel omakorda `x`-i järgi
    1. Mõlemad on sorteeritud esmalt `x` järgi aga esimeses andmestikus on seejärel sorteeritud y-i järgi kasvavalt, teises andmestikus on y-i järgi kahanevalt.
    1. Mõlemad on sorteeritud esmalt `x` järgi aga esimeses andmestikus on seejärel sorteeritud y-i järgi kahanevalt, teises andmestikus on y-i järgi kasvavalt.
    1. Mõlemad andmestikud on täpselt samamoodi järjestatud.
...
- **Ülesanne 2** Mis sorti iiris on sorteeritud andmestikus 30 kohal? Omista selle sordi nimi muutujale `kolmekymnes`.
...
```{r eval = F}
B <-  read.csv2(file = "https://github.com/Rkursus/mooc/raw/main/data/B.csv",   nrows = 160, stringsAsFactors = T)
...
Loe sisse andmestik [rotid](https://github.com/Rkursus/mooc/raw/main/data/rotid.csv) nimega `rotid`. 
