...
knitr::include_graphics("images/console1.png")
```
...
Leia oma arvutis programm RStudio, kui sa pole seda veel paigaldanud, siis saad selle tasuta alla laadida [RStudio kodulehelt](https://rstudio.com/products/rstudio/download/#download). **NB!** RStudio pole ainus R-i kasutajaliides, on veel palju teisi aga pidime valima ühe aga pea kõik kursusel käsitletav on tehtav igas teises R-i kasutajaliideses.
...
```{r kysimused1, echo=FALSE}
question("Mida teeb R kui koodirea ees on `#` märk?",
  answer("R käivitab käsu"),
  answer("See tähedab, et R-i käsk on poolik ja R-i ootab lisasisendit"),
  answer("See tähistab kommentaari märki ja R ei tee midagi", correct = TRUE),
  answer("See on _hastag_ ehk räsimärk ja seega määrab käsu tüübi või teema")
)
```

```{r kysimused2, echo=FALSE}
question("Mida R ootab kui käsurea ees on  `+` märk?",
  answer("R annab märku, et ta hetkel arvutab midagi"),
  answer("See tähedab, et R-i käsk on poolik ja R-i ootab lisasisendit", correct = TRUE),
  answer("See tähistab kommentaari märki ja R ei tee midagi"),
  answer("See on tavaline liitmise märk ja R summeerib eelmise ja järgmise käsu mis sa annad")
)
```
...
1. Arvuta enda kehamassiindeks: kaal (kg) / pikkus^2^ (m^2^).
...
### Väärtuste omistamine ja töökeskkond
